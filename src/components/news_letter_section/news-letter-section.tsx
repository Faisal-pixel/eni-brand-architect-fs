...
const newsletterConfig = {
  ...
  submissionEndpoint: "/api/v1/newsletter/subscribe",
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.error === "Email is already subscribed.") {
          setErrors({ email: "Email is already subscribed." });
        }
  ...
  return (
    <div ref={sectionRef} className="w-full">
      <div className="flex flex-col md:flex-row md:justify-between">
        {/* Partner logos */}
        <div
          className={` transition-all duration-1000 mb-2.5 flex items-center justify-center ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
        >
          <Image
            src={NewsLetterLeftLogos}
            alt="Newsletter Left Logo"
            className=""
          />
        </div>
        ...
        {/* Main content */}
        <div
          className={`text-center md:w-[592px] transition-all duration-1000 delay-300 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
        >
          ...
          {submitStatus === "success" && (
            <div className="max-w-md mx-auto mb-4 p-3 bg-green-100 border border-green-300 rounded-lg">
              <p className="text-green-800 text-sm">
                {newsletterConfig.messages.success}
              </p>
            </div>
          )}
          ...
          {submitStatus === "error" && (
            <div className="max-w-md mx-auto mb-4 p-3 bg-red-100 border border-red-300 rounded-lg">
              <p className="text-red-800 text-sm">
                {newsletterConfig.messages.error}
              </p>
            </div>
          )}
