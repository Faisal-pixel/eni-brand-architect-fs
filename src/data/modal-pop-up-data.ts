import { DateSection } from "@/app/types/modal-pop-up.types";

const MarketingAndAmplificationData: DateSection[] = [
  {
    date: "February 28, 2018",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "1",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "2",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "3",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "4",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "5",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "6",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "7",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "8",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "9",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "10",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v1",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
      {
        id: "v2",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_2mb.mp4",
        type: "video",
      },
    ],
  },
  {
    date: "February 28, 2019",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "11",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "12",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "13",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "14",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "15",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v3",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
    ],
  },
];
const MediaProductionAndEventCoverageData: DateSection[] = [
  {
    date: "February 28, 2018",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "1",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "2",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "3",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "4",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "5",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "6",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "7",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "8",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "9",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "10",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v1",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
      {
        id: "v2",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_2mb.mp4",
        type: "video",
      },
    ],
  },
  {
    date: "February 28, 2019",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "11",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "12",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "13",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "14",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "15",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v3",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
    ],
  },
];
const TEEData: DateSection[] = [
  {
    date: "February 28, 2018",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "1",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "2",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "3",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "4",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "5",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "6",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "7",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "8",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "9",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "10",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v1",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
      {
        id: "v2",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_2mb.mp4",
        type: "video",
      },
    ],
  },
  {
    date: "February 28, 2019",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "11",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "12",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "13",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "14",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "15",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v3",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
    ],
  },
];
const playWithStephenData: DateSection[] = [
  {
    date: "February 28, 2018",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "1",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "2",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "3",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "4",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "5",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "6",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "7",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "8",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "9",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "10",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v1",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
      {
        id: "v2",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_2mb.mp4",
        type: "video",
      },
    ],
  },
  {
    date: "February 28, 2019",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "11",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "12",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "13",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "14",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "15",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v3",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
    ],
  },
];
const talentvilleAfricaData: DateSection[] = [
  {
    date: "February 28, 2018",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "1",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "2",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "3",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "4",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "5",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "6",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "7",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "8",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "9",
        src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "10",
        src: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v1",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
      {
        id: "v2",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_2mb.mp4",
        type: "video",
      },
    ],
  },
  {
    date: "February 28, 2019",
    paragraph:
      "From audit to identity, we lay the groundwork for your brand's success. Whether you're launching a new venture or redefining an existing one, we create distinctive brands that connect with your target audience and stand out in competitive markets.",
    images: [
      {
        id: "11",
        src: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "12",
        src: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "13",
        src: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "14",
        src: "https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=400&h=400&fit=crop",
        type: "image",
      },
      {
        id: "15",
        src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=400&fit=crop",
        type: "image",
      },
    ],
    videos: [
      {
        id: "v3",
        src: "https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4",
        type: "video",
      },
    ],
  },
];

export {MarketingAndAmplificationData, MediaProductionAndEventCoverageData, talentvilleAfricaData, playWithStephenData, TEEData};